...
def random_walk(M, N):
    ...
    for j in range(0, M):
        ...
        for i in range(0, N):
            theta = 2 * math.pi * random_MLCG(0,1)
            x1 = math.cos(theta)
            y1 = math.sin(theta)
            x = x + x1
            y = y + y1
...
def gaussian_quadrature(fun, order, a, b):
    #determining roots and weights for particular degree/order of legendre polynomials
...
def pde_explicit(u0, n, delx, delt, time):
    k = 0.1
    #A matrix
    A = make_matrix(n,n)
    alpha = k*delt/(delx**2)
...
def random_MLCG(L_limit, U_limit, a = 572, m = 16381, x0 = 572):
    m1 = 0
